...
The default paths can also be overridden when launching:

```bash
MODEL_NAME=/path/to/Wan2.1-T2V-1.3B \
EVAL_CKPT=/path/to/nusc_trd.safetensors \
VAE_PATH=/path/to/dcae_td_53000.pkl \
...
Edit `config/horizondrive_unified_i2v_eval.yaml` if your dataset is stored
elsewhere. The current eval config uses the validation split, one front camera,
and computes `fid` and `fvd`:
...
Run the default single-GPU evaluation:
