...
from horizondrive.utils.prope_utils import get_prope_fn
...
class WanSelfAttention(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_prope:
            ...
            self.small_head_num = 1
            self.q_s = nn.Linear(dim, small_dim)
            self.k_s = nn.Linear(dim, small_dim)
            self.v_s = nn.Linear(dim, small_dim)
            self.o_s_prope = nn.Linear(small_dim, dim)
            nn.init.zeros_(self.o_s_prope.weight)
            if self.o_s_prope.bias is not None:
                nn.init.zeros_(self.o_s_prope.bias)
    ...
    def forward(
        ...
        if self.use_prope:
            n_s = self.small_head_num
            d_s = self.small_dim // n_s
            q_s = self.q_s(x.to(dtype)).view(b, s, n_s, d_s)
            k_s = self.k_s(x.to(dtype)).view(b, s, n_s, d_s)
            v_s = self.v_s(x.to(dtype)).view(b, s, n_s, d_s)
            apply_fn_o_s = None  # will be set when proj_mats are available

        if return_kv:
            k_cache, v_cache = k.clone(), v.clone()


        proj_mats = pos_emb_fn.get_projection_matrices() if self.use_prope else None
        if kv_cache is not None:
            ...
            if proj_mats is not None and self.use_prope:
                d_s = self.small_dim // self.small_head_num
                apply_fn_q_s, apply_fn_kv_s, apply_fn_o_s = get_prope_fn(head_dim=d_s, projmat=proj_mats)
                q_s = apply_fn_q_s(q_s)
                k_s = apply_fn_kv_s(k_s)
                v_s = apply_fn_kv_s(v_s)
        ...
        if proj_mats is not None and self.use_prope and apply_fn_o_s is not None:
            # small ProPE branch: cheap cam-relative attention at 1/8 dim
            x_s = attention(
                q=q_s,
                k=k_s,
                v=v_s,
            ...
            x_s = apply_fn_o_s(x_s)
            x = x + self.o_s_prope(x_s.flatten(2))
