...
class RopeEmb(nn.Module):
    ...
    def forward(self, q, k, grid_sizes):
        device = q.device
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            freqs(Tensor): Rope freqs, shape [1024, C / num_heads / 2]
        """
        e = (self.modulation + e).chunk(6, dim=-2)  # [B, 6/1, C] or [B, F, 6/1, C] or [B, F, H, W, 6/1, C]
        if len(x.shape) != 3:
            raise ValueError(f"invalid x shape: {x.shape}")
        
        if len(e[0].shape) == 3:
            modulation_mode = "global"   # [B, 1, C]
        elif len(e[0].shape) == 4:
            modulation_mode = "frame"    # [B, F, 1, C]
        elif len(e[0].shape) == 6:
            modulation_mode = "pixel"    # [B, F, H, W, 1, C]
        ...
        def _unflatten(x):
            # x: [B, L, C]
            if modulation_mode == "global":
                return x
            elif modulation_mode == "frame":
                return rearrange(x, "b (f h w) c -> b f (h w) c", f=grid_sizes[0][0], h=grid_sizes[0][1], w=grid_sizes[0][2])
            elif modulation_mode == "pixel":
                return rearrange(x, "b (f h w) c -> b f h w c", f=grid_sizes[0][0], h=grid_sizes[0][1], w=grid_sizes[0][2])
        ...
        def _flatten(x):
            if modulation_mode == "global":
                return x
            if modulation_mode == "frame":
                return rearrange(x, "b f hw c -> b (f hw) c")
            elif modulation_mode == "pixel":
...
class ViewIDEmbedding(nn.Module):
    ...
    def forward(self, x, y, **kwargs):
        ...
        nfhw = x[0].shape[1]
        fhw = nfhw // num_views
        # view_ids = torch.range(0, num_views - 1, device=device).float()
        view_ids = y.to(device).float()
        ...
        views_emb = rearrange(views_emb, 'n fhw d -> 1 nfhw d', nfhw=nfhw, n=num_views, fhw=fhw)
        x = [u + views_emb for u in x]  # (1, nfhw, d)
        return x
...
class UnifiedTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.action_embedding = nn.Sequential(
            nn.Linear(freq_dim * 2, dim), nn.SiLU(), nn.Linear(dim, dim))
        self.action_projection = nn.Sequential(nn.SiLU(), nn.Linear(self.dim, self.dim * 6))
    ...
    def forward(
        ...
        if len(t.shape) != 1 and len(t.shape) != 2 and len(t.shape) != 4:
            raise ValueError(f"t must be of shape [B] or [B, F] or [B, F, H, W], but got {t.shape}")
        ...
        grid_sizes = torch.stack(
            [torch.tensor(u.shape[2:], dtype=torch.long) for u in x])  # shape: (B, 3)

        x = [u.flatten(2).transpose(1, 2) for u in x]    # Each `u` has shape (1, nf*h*w, d).

        actions = None
        if "action" in additional_conditions:
            actions = additional_conditions["action"]
        
        for condition_name, y_add in additional_conditions.items():
            if condition_name in ["action"]:
                continue
        ...
        self.pos_emb_fn.set_projection_matrices(proj_mats)
        ...
        with torch.amp.autocast('cuda',dtype=torch.float32):
            t_shape = t.shape                    # Can be [B], [B, F], or [B, F, H, W].
            t_flat = t.reshape(-1)               # [N]

            # 1) Compute time embeddings on the flattened timesteps.
            emb_flat = sinusoidal_embedding_batchwise(self.freq_dim, t_flat).float()   # [N, D_freq]
            e_flat = self.time_embedding(emb_flat)                                      # [N, D_e]

            # Restore `e` to the original timestep shape.
            e = e_flat.view(*t_shape, -1)

            # 2) Project to the 6*C modulation parameters.
            e0_flat = self.time_projection(e_flat)          # [N, 6 * self.dim]
            e0 = e0_flat.unflatten(-1, (6, self.dim))       # [N, 6, C]

            # Restore `e0` to the original timestep shape as well.
            e0 = e0.view(*t_shape, 6, self.dim)

            if actions is not None:
                # a has to be scaled to 0-1000, while dx in[0, 2], dy in [-0.015, 0.015]
                actions = actions * torch.tensor([500.0, 60000.0], device=actions.device, dtype=actions.dtype)
                ...
                a0 = self.action_projection(a).unflatten(-1, (6, self.dim))  # [B, F, 6, C]
                if e.ndim == 5: 
                    a = a[:, :, None, None, :].expand(-1, -1, e.shape[2], e.shape[3], -1)
                    a0 = a0[:, :, None, None, :, :].expand(-1, -1, e0.shape[2], e0.shape[3], -1, -1)
        ...
        if step is not None and step != "final":
            prope_weight = max(0.0, min(1.0, (step-50.0) / (600-50.0)))
        else:
            prope_weight = 1.0
        self.pos_emb_fn.set_prope_weight(prope_weight)
            ...
            if not self.should_calc:
                ...
                for block in self.blocks:
                    ...
                    if torch.is_grad_enabled() and self.gradient_checkpointing:
                        ...
                        if layer_cur in collect_layer:
                            diff_latents.append(x)
                    else:
                        # arguments
                        kwargs = dict(
                            e=e0,
