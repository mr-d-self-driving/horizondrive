...
class WanFunUnifiedPipeline(DiffusionPipeline):
    ...
    # ------------------------------------------------------------------ #
    #  Main entry — multi-step rollout with optional sink frames
    # ------------------------------------------------------------------ #
    @torch.no_grad()
    ...
    def __call__(
        ...
        sink_frame_num: int = 0,
        sink_zero_condition: bool = True,
        ...
        Args:
            ...
            sink_frame_num: Number of global anchor frames extracted from step 0 and reused in all
                subsequent rollout steps.  Set to 0 to disable (original overlap-only rollout).
        Examples:
        Returns:
        The total condition region per window is ``sink_frame_num + num_condition_images``.
        """
        ...
        total_cond_frames = sink_frame_num + num_condition_images
        ...
        for unroll_step in tqdm(range(num_unroll_steps), desc="Rollout"):
            ...
            # ---- Step 0: encode GT video, build mask & sink ----
            if unroll_step == 0:
                if mask is not None:
                ...
                if video is not None:
                    ...
                    # Extract per-camera sink latents as global anchor
                    if sink_frame_num > 0:
                        f_per_cam = video_latents.shape[2] // num_cameras
                        sink_latents = torch.cat([
                            video_latents[:, :, i * f_per_cam : i * f_per_cam + sink_lat_num]
                            for i in range(num_cameras)
                        ], dim=2).to(dtype=dtype)
                else:
                    cond_latents = None
            ...
            # ---- Step > 0: build rolling condition latents ----
                ...
                # Prepend sink latents per camera (shared by both paths)
                if sink_latents is not None:
                    cf = cond_latents_tmp.shape[2] // num_cameras
                    cond_latents = torch.cat([
                        torch.cat([
                            sink_latents[:, :, i * sink_lat_num : (i + 1) * sink_lat_num],
                            cond_latents_tmp[:, :, i * cf : (i + 1) * cf],
                        ], dim=2)
                        for i in range(num_cameras)
                    ], dim=2)
                else:
                ...
                # Rebuild fill_mask: the GT mask from step 0 only marks
                # num_condition_images frames per camera (no sink), but
                # step > 0 cond_latents = sink + rolling = total_cond_frames
                # per camera, so we need a mask that matches that count.
                if sink_frame_num > 0:
                    nf_lat = latents.shape[2] // num_cameras
                    cond_mask = torch.zeros(
                        latents.shape[0], nf_lat,
                        dtype=torch.bool, device=latents.device,
                    )
                    cond_mask[:, :total_cond_frames] = True
                    # Replicate to (b, C, n*F, H, W) matching latents
                    fill_mask = cond_mask.unsqueeze(1).unsqueeze(-1).unsqueeze(-1).expand_as(latents)

            # ---- Build additional conditions for this window ----
            # When sink > 0 and unroll_step > 0, the rolling condition starts
            # Slice additional_conditions starting from begin_idx.
            # When sink > 0 the sink position (position 0) will be patched with
            # GT frame-0 conditions below, so position 1 naturally aligns with
            # GT frame (begin_idx + 1) which matches the rolling latent.
            ...
            for name, value in additional_conditions.items():
                if name in ["normal", "hdmap", "bbox"]:
                    if value.shape[1] >= (cond_begin_idx + num_frames) * num_cameras:
                        ...
                        # Sink frames always carry GT frame-0 conditions (not begin_idx),
                        # because the sink latent is always from GT frame 0.
                        if sink_frame_num > 0 and unroll_step > 0:
                            gt_value = rearrange(value, "b (n f) c h w -> (b n) f c h w", n=num_cameras)
                            cur_value[:, :sink_frame_num] = gt_value[:, :sink_frame_num]
                        if sink_frame_num > 0 and sink_zero_condition:
                            cur_value[:, :sink_frame_num] = 0.0
            ...
            if "action" in additional_conditions:
                ...
                if action.shape[1] >= (cond_begin_idx + num_frames) * num_cameras:
                    ...
                    # Sink frames always carry GT frame-0 conditions (not begin_idx).
                    if sink_frame_num > 0 and unroll_step > 0:
                        gt_action = rearrange(action, "b (n f) c -> (b n) f c", n=num_cameras)
                        cur_action[:, :sink_frame_num] = gt_action[:, :sink_frame_num]
                    if sink_frame_num > 0 and sink_zero_condition:
                        cur_action[:, :sink_frame_num] = 0.0
            ...
            if "proj_mats" in additional_conditions:
                cur_proj_mats = additional_conditions["proj_mats"]
                cur_proj_mats = rearrange(cur_proj_mats, "b (n f) ... -> (b n) f ...", n=num_cameras)
                cur_proj_mats = cur_proj_mats[:, cond_begin_idx:cond_begin_idx + num_frames].clone()
                cur_proj_mats = rearrange(cur_proj_mats, "(b n) f ... -> b (n f) ...", n=num_cameras)
                cur_proj_mats = sample_proj_mats(
                    cur_proj_mats, num_cameras=num_cameras, temporal_compression_ratio=t_compression_ratio,
                )
                cur_additional_conditions["proj_mats"] = cur_proj_mats.to(device=device)
