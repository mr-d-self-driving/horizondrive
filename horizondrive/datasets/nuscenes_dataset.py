...
def obtain_next2top(first, current, epsilon=1e-6, v2=True):
    if Quaternion is None:
        raise ImportError("pyquaternion is required for nuScenes pose transforms")
...
class nuScenesDataset(BaseDataset):
    ...
    def get_rollout_ann_entry(self, clip_id, idx=None, sample_data=None):
        clip_idx = None
        if idx is not None and 0 <= int(idx) < len(self.clip_infos):
            clip_idx = int(idx)
        if clip_idx is None:
            clip_idx = self.clip_token_to_clip_idx.get(clip_id)
        if clip_idx is None:
            return None
        return {
            ...
            "map_dataroot": self.map_dataroot,
    ...
    def __getitem__(
        ...
        if model_mode == "i2v":
            ...
            if not self.use_t_variant_noise:
